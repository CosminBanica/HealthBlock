...
import InstitutionsList from "./InstitutionsList.js";
import "./App.css";
...
class App extends Component {
  ...
  async loadBlockchainData() {
    ...
    if (healthBlockData) {
      const healthBlock = new web3.eth.Contract(
        HealthBlock.abi,
        healthBlockData.address
      );
    ...
    // Get accountType
    let isPatient = await this.state.healthBlock.methods
      .isPatient(this.state.account)
      .call();
    if (isPatient === true) {
      this.setState({ accountType: "patient" });
    } else {
      let isDoctor = await this.state.healthBlock.methods
        .isDoctor(this.state.account)
        .call();
      if (isDoctor === true) {
        this.setState({ accountType: "doctor" });
      } else {
        let isInstitution = await this.state.healthBlock.methods
          .isInstitution(this.state.account)
          .call();
  ...
  registerAsPatient = () => {
    ...
    this.state.healthBlock.methods
      .registerPatient()
  ...
  registerAsInstitution = (institutionName, institutionLink) => {
    this.setState({ loading: true });
    this.state.healthBlock.methods
      .registerInstitution(institutionName, institutionLink)
      .send({ from: this.state.account })
      .on("transactionHash", (hash) => {
        this.setState({ loading: false });
      });
    ...
    if (this.state.loading) {
      ...
      if (this.state.accountType === "none") {
        content = (
          <div>
            <p id="loader" className="text-center">
              Please register an account type to continue
            </p>
            <button
              type="submit"
        ...
        if (this.state.location === "instlist") {
          content = (
            <InstitutionsList
              account={this.state.account}
              accountType={this.state.accountType}
      ...
      <div>
        ...
        <div className="container-fluid mt-5">
          <div className="row">
            ...
            >
              <div className="content mr-auto ml-auto mt-3">{content}</div>
            </main>
          </div>
        </div>
      </div>
