...
class AccessList extends Component {
    ...
    async getAccessList() {
        let institutions = await this.props.healthBlock.methods.getPatientAccessList(this.props.account).call({from: this.props.account})
    ...
    addInstitution = async (address) => {
        this.props.healthBlock.methods.shareRecords(address).send({ from: this.props.account }).on('transactionHash', async (hash) => {
    ...
    removeInstitution = async (address) => {
        this.props.healthBlock.methods.unshareRecords(address).send({ from: this.props.account }).on('transactionHash', async (hash) => {
            window.location.reload()
        })
        window.location.reload()
    }
